...
export default function PlayerApp({ publicId }) {
  ...
  const load = async () => {
    try {
      ...
      if (registrationId && verified) {
        try { const player = await api(`/registrations/${registrationId}`, { authScope: 'player' }); setRegistration(player); if (player.status !== 'PENDING' || player.payment) setPage('status'); } catch { localStorage.removeItem(key); }
  ...
  return <main className="app"><Toast text={toast} clear={() => setToast('')} />
    ...
    {page === 'status' && <PlayerStatus match={match} registration={registration} back={() => setPage('match')} retry={() => setPage('pay')} refresh={async () => { save(await api(`/registrations/${registration.public_id}`, { authScope: 'player' })); setToast('Status updated'); }} toast={setToast} />}
  </main>;
}
