...
export default function PlayerStatus({ match, registration, back, retry, refresh, rejoin, onCancelled, toast }) {
  ...
  const confirmed = registration.status === 'CONFIRMED'; const submitted = registration.payment?.status === 'SUBMITTED'; const rejected = registration.status === 'REJECTED'; const waitlisted = registration.status === 'WAITLISTED'; const cancelled = registration.status === 'CANCELLED';
