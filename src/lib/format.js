...
export function playerStatus(item) { if (item?.status === 'CANCELLED') return 'CANCELLED'; if (item?.payment?.status === 'SUBMITTED') return 'PAYMENT_SUBMITTED'; return item?.status; }
